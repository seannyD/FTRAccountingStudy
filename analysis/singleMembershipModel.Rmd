...
Run an OLS regression, then print the results when clustering standard errors by language family.  This uses code from Beautrelet (2018) and Hlavac (2018).

Beautrelet, I. (2018) https://github.com/IsidoreBeautrelet/economictheoryblog
...
reg <- lm(AAM.scaled ~
             ...
             MEET + LOSS, 
             data=d2)
sStandard = summary(reg)
sCluster = summary(reg,cluster = c("mainLanguageFamily"))
...
htmlTable =
  stargazer(reg,reg,se=list(
  coef(sStandard)[, 2],
  coef(sCluster)[, 2]
  ), type = "html", out = "../results/modelResults/RobustClusterResults.html")
```
...
clusteredAndRobust = ols(AAM.scaled ~
             strongftr, 
             data=d2, robust = T, cluster = "mainLanguageFamily")
```
